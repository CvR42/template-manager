...
\end{verbatim}
\end{showfile}
\subsection{Class hierarchy inquiry functions}
...
\end{verbatim}
\end{desc}
\subsection{Invoking the analyser template}
After the macros describe in the previous section have been defined,
...
\par
\begin{desctab}
\item[\texttt{basename}]\index{basename@\texttt{basename}}
...
\item[\texttt{wantdefs}]\index{wantdefs@\texttt{wantdefs}}
...
\item[\texttt{notwantdefs}]\index{notwantdefs@\texttt{notwantdefs}}
If set, this variable contains the names of the type and function
definitions for which under \emph{all} conditions \emph{no} code must
be generated.

\item[\texttt{alldefs}]\index{alldefs@\texttt{alldefs}}
