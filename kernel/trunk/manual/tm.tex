...
\item[\texttt{bits n1..nn}\index{bits@\texttt{bits}}]
Return the number of bits required to represent each
of the values of $\texttt{n1} \cdots \texttt{nn}$.
A value must be non-negative. The given number is assumed to be
part of a range of values starting at 0, so the value
$3$ requires $2$ bits, and the value $4$ requires $3$ bits.
